...
public class Torneo {
	...
	public void realizarTorneo() throws FaltanEntrenadoresException, RequisitosPokemonesException {
		...
		Entrenador ganador;
		if (cantidadEntrenadoresNecesaria()) {
			noCumpleRequisitos = noTieneMinimoUnPokemon();
			if (noCumpleRequisitos == null) {
				inicializarHechizosDisponibles();
				System.out.println("\nSE DISPUTAN LOS CUARTOS DE FINAL DEL TORNEO");
				semifinalistas = ejecutarRonda(entrenadores);
				System.out.println("\nSE DISPUTAN LAS SEMIFINALES DEL TORNEO");
				finalistas = ejecutarRonda(semifinalistas);
				System.out.println("\nSE DISPUTA LA FINAL DEL TORNEO");
				ganador = ejecutarFinal(finalistas);
				System.out.println("\nEL ENTRENADOR " + ganador.getNombre() + " HA SIDO EL GANADOR DEL TORNEO");
				restaurarPokemones();
			} else
				throw new RequisitosPokemonesException(
						"Al menos uno de los entrenadores no cumple con las condiciones del torneo", 1,
						noCumpleRequisitos.cantidadPokemones(), noCumpleRequisitos);
		} else
	...
	private Entrenador[] ejecutarRonda(Entrenador[] entrenadoresParticipantes) {
		...
		Pokemon poke1, poke2;
		Hechizo hechizo1, hechizo2;
		Entrenador[] entrenadoresGanadores = new Entrenador[participantes / 2];
		for (i = 0; i < participantes; i += 2) {
			System.out.println("\n" + entrenadoresParticipantes[i].getNombre() + " VS "
					+ entrenadoresParticipantes[i + 1].getNombre());
			poke1 = seleccionarPokemon(entrenadoresParticipantes[i]);
			poke2 = seleccionarPokemon(entrenadoresParticipantes[i + 1]);
	 ...
	 * Pre: El entrenador es no nulo y tiene al menos un Pokemon<br>
	 */
	...
	/**
	 * Por comandos decidimos el Hechizo que usara el entrenador al principio de la ronda<br>
	 ...
	 * @throws HechizoNoEncontradoException Si el hechizo con dicho nombre no existe<br>
	 */
	 ...
	 * Se elije al azar quien va primero. Luego cada pokemon ejecuta su secuencia de ataque<br>
	 * Se calcula el puntaje de cada uno. El ganador sera el que tenga mayor puntaje<br>
	 * @return Ganador<br>
	...
	private void aniadirEnfrentamiento(Entrenador entrenadorGan, Pokemon pokeGan, Hechizo hecGanador,
	...
	/**
	 * El puntaje se calcula como la suma del escudo + vitalidad + el 50% de la fuerza<br>
	 ...
	 * @throws MaximaCapacidadEntrenadoresException Si se agrega mas de 8 entrenadores<br>
	 */
	public void aniadirEntrenador(Entrenador entrenador) throws MaximaCapacidadEntrenadoresException, EntrenadorRepetidoException {
		if (entrenador != null) {
